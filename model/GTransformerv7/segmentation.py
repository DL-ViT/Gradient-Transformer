...
class UpBlock_attention(nn.Module):
    def __init__(self, in_channels, out_channels, nb_Conv, activation='ReLU',scale_factor=2):
        super().__init__()
...
class GTransformerv7(nn.Module):
    def __init__(self,  n_channels=1, n_classes=1, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        self.contras5 = _NonLocalBlockND(in_channels=in_channels*8,inter_channels=in_channels)
        self.cattn = eca_layer(channel=in_channels*8)
    ...
    def forward(self, x):
        ...
        d6 = self.encoder5(self.pool(x5))
        # Transfor_layer
        c1 = self.contras1(x1)
        c2 = self.contras2(x2)
        c3 = self.contras3(x3)
        c4 = self.contras4(x4)
        c5 = (self.contras5(x5)+ x5*self.cattn(x5))/2
        # decoder
